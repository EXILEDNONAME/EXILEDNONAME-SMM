  ...
  **/
  ...
  public function index() {
    ...
    foreach($item as $item) {
      if ($now - strtotime($item->created_at) > 300) { $this->model::where(['id' => $item->id, 'id_product' != '9'])->update(['status' => 3]); }
      else if ($now - strtotime($item->created_at) > 60) { $this->model::where(['id' => $item->id, 'id_product' != '9'])->update(['status' => 2]); }
